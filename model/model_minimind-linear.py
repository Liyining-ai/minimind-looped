...
class LinearAttention(nn.Module):
    ...
    def forward(self, x, position_embeddings, past_key_value=None, use_cache=False, attention_mask=None):
        ...
        if seq_len > 1:
            ...
            # === 归一化用未加 RoPE 的 q,k ===
            k_cumsum = k.cumsum(dim=-2)  # (bsz, heads, seq_len, head_dim)
            
            # q: (bsz, heads, seq_len, head_dim)
            denom = (q * k_cumsum).sum(dim=-1, keepdim=True).clamp(min=1e-6)  # (bsz, heads, seq_len, 1)
            scores = scores / denom
            ...
            if use_cache:
                kv_sum = k_rope.transpose(-1, -2) @ v
                k_sum = k.sum(dim=2, keepdim=True)  # 注意归一化项累积的是未加 RoPE 的 k
                cache = (kv_sum, k_sum)
            else:
                cache = None
        else:
            ...
            # 单步 q,k,v
            q_rope_t, k_rope_t, v_t = q_rope[:, :, 0, :].to(torch.float32), k_rope[:, :, 0, :].to(torch.float32), v[:, :, 0, :].to(torch.float32)
            q_t, k_t = q[:, :, 0, :].to(torch.float32), k[:, :, 0, :].to(torch.float32)
        
            kv_sum = kv_sum + torch.einsum("bhd,bhe->bhde", k_rope_t, v_t)
            k_sum = k_sum + k_t  # 注意归一化项累积的是未加 RoPE 的 k
        
            denom = (q_t * k_sum).sum(dim=-1, keepdim=True).clamp(min=1e-6)
            out_step = (q_rope_t @ kv_sum) / denom
...
class DWCLinearAttention(nn.Module):
    def __init__(self, args: MiniMindConfig):
        ...
        self.resid_dropout = nn.Dropout(args.dropout)
        self.lepe = nn.Conv1d(
        in_channels=self.n_local_heads * self.head_dim,
        out_channels=self.n_local_heads * self.head_dim,
        kernel_size=6,
        padding=5,    # 为了 causal，只看自己+前面
        groups=self.n_local_heads * self.head_dim,
        bias=False)
    ...
    def forward(self, x, position_embeddings, past_key_value=None, use_cache=False, attention_mask=None):
        ...
        lepe_out = self.lepe(v_lepe)[..., :seq_len]                           # causal 保证
